    ...
    <?php
      ...
      if(isset($_GET['id'])){
        ...
        if($stmt->rowCount() > 0) {
          ...
          $stmt = $db->prepare("SELECT question.id, question.content, answer.content AS answercontent FROM question JOIN answer ON question.id = answer.qid WHERE question.qid = ? ORDER BY answer.id DESC");
          $stmt->execute(array( $_GET['id']));
          if($stmt->rowCount() > 0) {
        ...
        $stmt = $db->prepare("SELECT question.id, question.content FROM question WHERE question.written = 1 AND question.qid = ? ORDER BY question.id DESC");
        $stmt->execute(array( $_GET['id']));
        if($stmt->rowCount() > 0) {
          $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
          echo '<hr/> <div class="row">';
